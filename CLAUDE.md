# AGENTS.md - AI Agent Development Guide for tap-weatherapi
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from WeatherAPI.

### Key Components

1. **Tap Class** (`tap_weatherapi/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_weatherapi/client.py`): Handles API communication and authentication
1. **Streams** (`tap_weatherapi/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_weatherapi/streams.py`
...
```python
class MyNewStream(WeatherAPIStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(WeatherAPIStream):
...
```bash
tap-weatherapi --config config.json --discover
...
1. Update `config_jsonschema` in `tap_weatherapi/tap.py`
...
```bash
# .env.example
TAP_WEATHERAPI_API_URL=https://api.example.com
TAP_WEATHERAPI_API_KEY=your_api_key_here
TAP_WEATHERAPI_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-weatherapi --version`), Python version, and a minimal reproduction case when reporting bugs.
